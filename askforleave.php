...
<?php
	...
	if($DataTime==""||$reason=="")
	{
		...
		echo "<script>document.location = 'inschool.php';</script>";
		die;
	}
	else{
		...
		if($perm){
			...
			if($insert_result){
				echo "<script>alert('请假成功！'+</br>+'请尽早销假!');</script>";
				echo "<script>document.location = 'outschool.php';</script>";
			}
			else{
				echo "<script>alert('请假失败，请联系管理员！');</script>";
				echo "<script>document.location = 'inschool.php';</script>";
		...
		{
			echo "<script>alert('您有未销假信息！');</script>";
			echo "<script>document.location = 'outschool.php';</script>";
		}
	}
?>
